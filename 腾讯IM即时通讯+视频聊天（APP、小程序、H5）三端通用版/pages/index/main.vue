<template>
	<div class="chat-container">
		...
		<template v-for="item in allConversation">
			<div class="chat" v-if="item.type === 'C2C'" :key="item.conversationID" @longpress="longTimePress(item)" @click="checkoutConversation(item, item.userProfile.nick || item.userProfile.userID)">
			...
			<div class="chat" v-else-if="item.type === 'GROUP'" @click="checkoutConversation(item, item.groupProfile.name || item.groupProfile.ID)"
			...
			<div class="chat" v-else-if="item.type === '@TIM#SYSTEM'" @click="checkoutNotification(item)" :key="item.conversationID"
