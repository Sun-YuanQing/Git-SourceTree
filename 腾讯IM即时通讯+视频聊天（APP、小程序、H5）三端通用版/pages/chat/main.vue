<template>
	<div class="chat" id="chat" :style="{ paddingBottom: isIphoneX ? (safeBottom + 70) + 'px': '70px' }" @longpress="handleLongPress"
	 @touchmove="handleTouchMove" @touchend="handleTouchEnd">
		...
		<div id="list" @click="loseFocus">
			<div v-for="(message, index) in currentMessageList" :key="index" :id="message.ID">
				<!--        系统消息-->
				<div class="notice" v-if="message.type === 'TIMGroupTipElem' || message.type === 'TIMGroupSystemNoticeElem'">
					<div class="content">
						<span v-for="(div, index1) in message.virtualDom" :key="index1">
						...
						<div class="wrapper" @longpress="handleMessage(message)">
							...
							<div class="message" v-if="message.type === 'TIMTextElem'">
								<div class="text-message">
									<span v-for="(div, index2) in message.virtualDom" :key="index2">
							...
							<div class="message" v-else-if="message.type === 'TIMFileElem'">
								<div class="file" @click="handleDownload(message.payload)">
									<!-- <iavatar src="../../static/images/file.png" size="large" shape="square" /> -->
                  <image style="width:100rpx;height:100rpx;border-radius:50%" src="../../static/images/file.png"/>
							...
							<div class="message" v-else-if="message.type === 'TIMCustomElem'">
								<div v-if="message.payload.data === 'survey'" class="survey">
									...
									<div class="description">
										<!-- <irate disabled="true" :value="message.payload.description"></irate> -->
									</div>
							...
							<div class="message" v-else-if="message.type === 'TIMVideoFileElem'">
								<video :src="message.payload.videoUrl" class="video" :poster="message.payload.thumbUrl" object-fit="contain"
					...
					<div>
						<!-- 自己发的消息头像 -->
            <image style="width:100rpx;height:100rpx;border-radius:50%" v-if="message.flow === 'out'" :src="myInfo.avatar" mode="" />
						<!-- <iavatar i-class="avatar" v-if="message.flow === 'out'" :src="myInfo.avatar" shape="square" /> -->
						<!-- 收到的消息头像 -->
            <image style="width:100rpx;height:100rpx;border-radius:50%" v-else-if="currentConversation.type === 'C2C'" @click="toDetail" :src="currentConversation.userProfile.avatar" />
            <image style="width:100rpx;height:100rpx;border-radius:50%" v-else @click="toDetail({ userID: message.from })" :src="message.avatar" />
						<!-- <iavatar i-class="avatar" shape="square" v-else-if="currentConversation.type === 'C2C'" @click="toDetail" :src="currentConversation.userProfile.avatar" /> -->
						<!-- <iavatar i-class="avatar" shape="square" v-else @click="toDetail({ userID: message.from })" :src="message.avatar" /> -->
					</div>
				</div>
			</div>
		</div>
		...
		<div class="bottom" :style="{ paddingBottom: isIphoneX ? safeBottom + 'px': '' }">
          ...
          <image style="width:50rpx;height:50rpx" :src="!isRecord ? '../../images/audio.png' : '../../images/record.png'" mode="" />
				</div> -->
				<div v-if="!isRecord" style="width: 100%">
					<input type="text" class="input" v-model.lazy:value="messageContent" confirm-type="send" cursor-spacing="10"
					 :focus="isFocus" @focus="isFocus = true" @blur="isFocus = false" @confirm='sendMessage' />
				</div>
          ...
          <image style="width:50rpx;height:50rpx" src="@/static/images/smile.png" mode="" />
				</div>
				<div class="send" @click="sendMessage" v-if="messageContent.length !== 0">
					发送
				</div>
				<div class="btn" @click="handleMore()" v-else>
          <image style="width:50rpx;height:50rpx" src="@/static/images/more.png" mode="" />
				</div>
			...
			<div class="bottom-emoji" v-if="isEmojiOpen">
				...
				<div class="emojis" v-if="emojiShow">
					<div v-for="(emojiItem, index3) in emojiName"   class="emoji" :key="index3" @click="chooseEmoji(emojiItem)">
		...
		<div class="float-button-list">
			<image src="../../static/images/camera.png" class="video-icon" v-if="currentConversation.type === 'C2C'" @click="videoCall">
			<image src="../../static/images/conversation-profile.png" @click="toDetail">
		</div>
	</div>
</template>
